...
export async function POST(request: NextRequest) {
  ...
  const { data: thread } = await supabase
    .from("chat_threads")
    .select("id, objective_id, model_id, objectives!inner(id, title, brief_md, publish_mode, company_id, companies!inner(id, name, industry, description, website_url, brief_general_md, account_id))")
    .eq("id", body.threadId)
    .maybeSingle();
  ...
  const objective = (thread as unknown as {
    ...
    objectives: {
      ...
      company_id: string;
      companies: { id: string; name: string; industry: string | null; description: string | null; website_url: string | null; brief_general_md: string | null; account_id: string };
    };
  }).objectives;
  ...
  const system = orchestratorSystemPrompt({
    ...
    companyBriefGeneral: objective.companies.brief_general_md,
